...
fun addUserToChatHandler(chatInputPort: ChatInputPort, userInputPort: UserInputPort): HttpHandler = {
            ...
            Jackson.asFormatString(
                chatInputPort.getAllChats().map {
                    ChatDto(
                        ...
                        messages = it.getMessages().map { message ->
                            MessageDto(
...
fun addChatHandler(chatInputPort: ChatInputPort, userInputPort: UserInputPort): HttpHandler = { request ->
    ...
    if (owner == null) {
        ...
        val chat = chatInputPort.createNewChat(
            owner = owner.id,
...
fun addMessageToChatHandler(chatInputPort: ChatInputPort): HttpHandler = {
    ...
    if (chat == null) {
        ...
        Response(status = Status.ACCEPTED).body("Message '${newChatMessageDto.message}' added to chat '${newChatMessageDto.chatId}'")
    }
